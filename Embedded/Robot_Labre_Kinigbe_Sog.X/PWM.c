    ...
    if(Abs(vitesseEnPourcents)<=100)
    {
        ...
        if(Motor == MOTEUR_GAUCHE)
        {
            if (vitesseEnPourcents >= 0){
                ...
                MOTEUR_GAUCHE_DUTY_CYCLE =  Abs(robotState.vitesseGaucheCommandeCourante*PWMPER); //Mise ‡1 de la pin;
            } 
        }
        if(Motor == MOTEUR_DROIT)
        {
            ...
            else if (vitesseEnPourcents >= 0)
            {
                ...
                MOTEUR_DROIT_DUTY_CYCLE =  Abs(robotState.vitesseDroiteCommandeCourante*PWMPER); //Mise ‡1 de la pin;
            } 
        }
    }
}
...
void PWMUpdateSpeed()
{
    // Cette fonction est Èappele sur timer 1 et permet de suivre des rampes d?ÈÈacclration
